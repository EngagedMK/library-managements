...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['returnBook'])) {
    $idMuonTra = (int)$_POST['idMuonTra'];
    $idTaiLieu = (int)$_POST['idTaiLieu'];
    $daysLate = (int)$_POST['daysLate'];
    $tienCoc = (float)$_POST['tienCoc'];
    
    if ($daysLate > 0) {
        
        $fine = $daysLate * 25000;
        $conn->query("UPDATE MuonTra SET tienPhat = $fine WHERE idMuonTra = $idMuonTra");

        // Tạo URL QR thanh toán
        $bank = "MB";
        $accountName = "Nguyen Xuan Khanh";
        $accountNumber = "6688888882003";
        $note = urlencode("Tra sach qua han");
        $qrUrl = "https://img.vietqr.io/image/{$bank}-{$accountNumber}-compact.png?amount={$fine}&addInfo={$note}";
        ?>
        <div class="overlay" id="qrModal" onclick="closeModal(event)">
            <div class="qr-modal" onclick="event.stopPropagation()">
                <h4>Trả sách quá hạn!</h4>
                <p>Số ngày trễ: <?php echo $daysLate; ?> ngày.</p>
                <p>Phí phạt: <?php echo number_format($fine); ?> đồng.</p>
                <p>Vui lòng thanh toán qua mã QR:</p>
                <img src="<?php echo $qrUrl; ?>" alt="QR Code">
                <form method="POST" action="">
                    <input type="hidden" name="confirmReturn" value="1">
                    <input type="hidden" name="idMuonTra" value="<?php echo $idMuonTra; ?>">
                    <input type="hidden" name="idTaiLieu" value="<?php echo $idTaiLieu; ?>">
                    <button type="submit" class="btn btn-success mt-2">Xác nhận đã thanh toán</button>
                </form>
                <button class="close-btn" onclick="closeModal()"><i class='bx bxs-x-square' ></i></button>
            </div>
        </div>
        <?php
    } else {
        ?>
        <div class="overlay-success" id="successModal" onclick="closeSuccessModal(event)">
            <div class="alert alert-success" onclick="event.stopPropagation()">
                <h4>Trả sách đúng hạn!</h4>
                <p>Số tiền cọc được hoàn lại: <?php echo number_format($tienCoc); ?> đồng.</p>
                <form method="POST" action="">
                    <input type="hidden" name="confirmReturn" value="1">
                    <input type="hidden" name="idMuonTra" value="<?php echo $idMuonTra; ?>">
                    <input type="hidden" name="idTaiLieu" value="<?php echo $idTaiLieu; ?>">
                ...
                <button class="close-btn" onclick="closeSuccessModal()"><i class='bx bxs-x-square' ></i></button>
            </div>
        </div>
        <?php
    }
...
// Xác nhận trả sách
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirmReturn'])) {
    $idMuonTra = (int)$_POST['idMuonTra'];
    $idTaiLieu = (int)$_POST['idTaiLieu'];

    // Cập nhật trạng thái và số lượng tài liệu
    $conn->query("UPDATE MuonTra SET trangThai = 'Đã trả' WHERE idMuonTra = $idMuonTra");
    $conn->query("UPDATE TaiLieu SET soLuong = soLuong + 1 WHERE idTaiLieu = $idTaiLieu");

    echo "<script>alert('Trả sách thành công!');</script>";
    header("Location:bookBorrowManagement.php?status=success&message=xác nhận thành công!!");
    exit();
}
?>
    ...
    <div class="body_div">
        ...
        <main>
            <div class="container mt-3">
                ...
                <div class="card mb-4">
                    <div class="card-body">
                        <h2 class="pt-3 pb-4 text-center font-bold font-up deep-purple-text">Danh sách sách đang mượn</h2>
...
<script>
    ...
    // // Tự động mở modal (tùy chỉnh nếu cần)
...
function closeSuccessModal(event) {
    const overlaySuccess = document.getElementById('successModal');
    if (!event || event.target === overlaySuccess) {
